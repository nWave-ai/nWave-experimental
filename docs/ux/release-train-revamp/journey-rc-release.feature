Feature: RC Release / Promote to Beta (Stage 2)
  ...
  Background:
    Given the nwave-dev repo has tag "v2.18.0.dev3"
    And the commit behind "v2.18.0.dev3" is "abc123d"
    And no existing RC tags for version "2.18.0"
  ...
  Scenario: Happy path - promote dev to first RC
    ...
    And Mike sets source_dev_tag to "v2.18.0.dev3"
    ...
    Then the RC version is "2.18.0rc1"
    ...
    Then tag "v2.18.0rc1" exists on nwave-dev
    And a GitHub pre-release is created for "v2.18.0rc1"
    When the pipeline publishes to PyPI
    Then "nwave-ai" version "2.18.0rc1" is available on production PyPI
    ...
    And the nWave-beta commit message contains "Dev tag: v2.18.0.dev3"
    And tag "v2.18.0rc1" exists on nWave-beta
  ...
  Scenario: Dry run validates all logic but produces no side effects
    ...
    Then all check-runs on "abc123d" are green
    When the pipeline calculates the RC version
    Then the RC version is calculated as "2.18.0rc1"
    ...
    And the pipeline reports "Would have: tagged v2.18.0rc1, published 2.18.0rc1 to PyPI, synced to nWave-beta"
  ...
  Scenario: Sequential RC counter increments correctly
    Given an RC tag "v2.18.0rc1" already exists
    When Mike triggers RC promotion from "v2.18.0.dev5"
    Then the RC version is "2.18.0rc2"
    And tag "v2.18.0rc2" is created

  Scenario: Source dev tag does not exist
    Given Mike triggers RC promotion with source_dev_tag "v2.18.0.dev99"
    And tag "v2.18.0.dev99" does not exist
    When the pipeline validates the source tag
    Then the pipeline fails with "Tag v2.18.0.dev99 not found"
    And the error message lists available dev tags: "v2.18.0.dev1, v2.18.0.dev2, v2.18.0.dev3"
    And no RC tag is created
    And nothing is published to PyPI

  Scenario: CI failed on source commit blocks RC promotion
    Given Mike triggers RC promotion from "v2.18.0.dev3"
    And the commit behind "v2.18.0.dev3" has a failed CI check-run
  ...
  Scenario: CI still running on source commit blocks RC promotion
    Given Mike triggers RC promotion from "v2.18.0.dev3"
    And the commit behind "v2.18.0.dev3" has a pending CI check-run
    When the pipeline checks CI status via GitHub API
  ...
  Scenario: No CI run found on source commit blocks RC promotion
    Given Mike triggers RC promotion from "v2.18.0.dev3"
    And the commit behind "v2.18.0.dev3" has no check-runs registered
    When the pipeline checks CI status via GitHub API
    Then the pipeline fails with "No CI run found for abc123d"
    And no RC tag is created

  Scenario: PyPI version collision
    Given "nwave-ai" version "2.18.0rc1" already exists on PyPI
    When the pipeline attempts to publish "2.18.0rc1"
    Then the pipeline logs a warning "v2.18.0rc1 already on PyPI"
  ...
  Scenario: Beta tester installs RC from PyPI
    Given "nwave-ai" version "2.18.0rc1" is published on production PyPI
    When a beta tester runs "pip install --pre nwave-ai==2.18.0rc1"
    Then nwave-ai 2.18.0rc1 is installed successfully
    And "nwave-ai version" shows "2.18.0rc1"

  Scenario: Cross-repo traceability is complete
    Given the RC promotion from "v2.18.0.dev3" completed successfully
