...
5. **User-centric walking skeleton — ONE per feature**: the skeleton delivers observable user value E2E -- answer "can a user accomplish their goal?" not "do the layers connect?" **Exactly ONE `@walking_skeleton` subprocess-e2e per FEATURE** (never per slice, never per command); every other scenario drives IN-PROCESS/in-memory. Wiring coverage is a declared triple, not scenario multiplication: (a) the feature's single skeleton proves the installed wiring once; (b) Vera's EXAMINE exercises every charter observable through the REAL surface (the user-perspective manual test); (c) the feature-end cycle (env-e2e + full-suite + deep-review) backstops paths no observable reaches. Additional E2E scenarios require an explicit justification (e.g. the slice's value IS an integration). Load `nw-test-design-mandates` for litmus test.
...
10. **Test file names carry BUSINESS VALUE, never process metadata.** A file named `test_slice_01_shared_git_worktree_refusal.py` embeds the delivery slice number in the name — meaningless once the slice ships, and meaningless to anyone reading the test tree without the feature-delta open beside it. Name the file after what it VERIFIES (`test_shared_git_worktree_refusal.py`), never after WHEN/under-which-slice it was authored. The slice number belongs in the commit trailer (`Slice-Id:`) and the ledger — never in a filename that outlives the slicing decision that produced it. Applies to every test file this agent authors or hands to a crafter to extend, including bugfix regression tests.
