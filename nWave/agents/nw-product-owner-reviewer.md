...
Goal: produce deterministic, structured YAML review feedback gating handoff to DESIGN wave -- approve only when journey artifacts are coherent, all 9 DoR items pass, every story has JTBD traceability (`job_id`), every slice contains at least one user-visible value story, and zero antipatterns remain.
...
3. **DoR is a hard gate**: No story proceeds to DESIGN without all 9 DoR items passing. One failure blocks entire handoff.
...
1. Check all journey dimensions and all 9 DoR items on every full review. Partial reviews use dimension-specific commands.
