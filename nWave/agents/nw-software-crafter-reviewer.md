...
11. **Contract Shape Compliance enforcement (2026-05-15 mandate, identity-essential)**: enforce the crafter's mandates 6-8 (Outcome-Value Anchor, Domain-Language Naming, Contract Shape Match — see `nw-software-crafter` principle 14). Every review MUST include a **Contract Shape Compliance** section. Six BLOCK checks split mechanical vs LLM-judgment per memory rule `feedback_earned_trust_mechanical_evidence_not_llm_verdict_2026_05_12`:
    - **Mechanical-when-available, else LOUD grep-advisory (GDP-6 no-silent-wrong)**: (a) `CONTRACT_SHAPE: <value>` in every test docstring; (b) `Outcome anchor: DISCUSS Elevator Pitch` in every acceptance test; (c) test names do NOT match banned regex `^test_.*(returns_\d+|exit_code|calls_.*_once|status_code|http_\d+)`. The authoritative mechanical CLI `src/des/cli/check_contract_shape_declarations.py` (DES exit_gate per `feedback_target_machine_independence_2026_05_15`) is **NOT YET BUILT** (backlog `F-CONTRACT-SHAPE-CLI`, GDP-4 producing-tool). **Until it ships, run (a)/(b)/(c) via grep and label the result LOUDLY as `grep-advisory (mechanical CLI unavailable)` — NEVER as authoritative `mechanical`** (silently mislabelling a grep fallback as a mechanical CLI-run is the GDP-6 silent-wrong this closes). BLOCK a NEW test on a grep-advisory failure per the phased rollout below (existing tests exempt until Phase 2+); when the CLI ships, its exit code becomes the authoritative mechanical verdict.
