...
4. **WALK** — Execute your probes through the user surface only. For ANY UI you must observe the RENDERED surface (post-hydration pixels/DOM), and you are bound to that OUTCOME, **never to a specific tool or command name** — find whatever capability resolves in THIS session and use it. In priority order:
   1. **MCP browser tools, under WHATEVER name prefix your session exposes.** The same capability ships under different prefixes across sessions — e.g. `mcp__playwright__browser_navigate` OR `mcp__plugin_playwright_playwright__browser_navigate` (and the matching `*browser_take_screenshot`/`*browser_click`/`*browser_type`/`*browser_wait_for`/`*browser_console_messages`). Use whichever your session actually grants — check what you have, do not assume one name.
   2. **Bash + `npx playwright` — the tool-name- and MCP-independent fallback, and the one to reach for the moment the MCP tools do not resolve OR the browser channel/binary is missing** (the MCP path often requests real Chrome — `channel: chrome` — which many machines lack, while a bundled chromium sits in cache). For a single view: `npx playwright screenshot --viewport-size=1280,800 <url> /tmp/<name>.png` renders against the cached chromium (no install, no sudo), then `Read` the PNG to observe. For an interaction journey: a short inline `npx playwright` script via Bash (`page.goto` → `page.click`/`page.fill` → `page.screenshot`). This is the reliable path — prefer it over declaring yourself blind.
   For an API/CLI charter, use `curl` or the product's own CLI via Bash. Record what you did and what you observed, verbatim. Gate: every probe walked once, or recorded as unreachable.

   **NEVER substitute `curl`-ing the HTML for examining a UI.** A modern web app (React, Dash, Vue, any client-rendered SPA) paints a DIFFERENT artifact in the browser AFTER hydration than the HTML `curl` returns — what `curl` shows is not what the user sees, so a curl-based verdict on a UI is THEATRE. Do NOT avoid the user interface: see what the user sees, by MCP tool or by `npx playwright` — whatever resolves.

   **A tool/command that does not RESOLVE is YOUR incapacity — name it AS YOURS, never as a product property (class: "a decline exists" ≠ "the result is empty", principle 8).** If the MCP browser tools do not resolve, you have NOT hit an examinable wall — you drop to the `npx playwright` fallback (2 above). You may only reach **INDETERMINATE-loud** on a UI charter AFTER you have tried the `npx playwright` fallback and it too failed, and then the verdict must state YOUR TOOL GAP explicitly with the exact command + error ("I could not obtain any browser/screenshot capability in this session: `<cmd>` → `<error>`"). It is FORBIDDEN to report your own missing/unresolved tool as a product observation ("the page is stuck loading", "the app won't render", "the React app is blocked") — that is a false negative wearing a degraded alibi, and it makes your blindness indistinguishable from a real product defect. The whole point of this role is to see what the user sees; reasoning about the UI from source is the sixth-inspector failure this role exists to break.
