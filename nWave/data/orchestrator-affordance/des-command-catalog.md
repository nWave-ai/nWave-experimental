...
- ⚠️ `des next --feature-id <id> [--format json]` — **PARTIAL: shipped at slice-01 of its plan; DO NOT treat its answer as authoritative.** It projects the next atdd_pure DELIVER-loop step (DISTILL the next slice · GREEN via crafter · examine · commit-slice), but it derives "which slice is pending" from the feature-delta's markdown `Status` column ALONE and never consults the AT-completion ledger. **Known lie**: for a slice that is committed and ledger-`SliceCommitVerified` while its markdown row still reads `pending`, it reports "acceptance tests are not yet authored" and its `how` tells you to re-author already-green, shipped ATs — following it DESTROYS delivered work. It also raises a bare `NotImplementedError` on a fully EXAMINE-verified slice. The designed-but-unbuilt behaviour (ledger-vs-markdown disagreement → INDETERMINATE naming the drift) is slice-02/03 scope, not yet delivered.
- **THE LEDGER IS THE TRUTH, not this tool.** When `des next` contradicts `.nwave/telemetry/atdd-pure/<feature-id>.jsonl`, believe the ledger. NEVER auto-execute the returned `how` — and never paste a `/nw-distill` it emits for a slice you know is committed.
...
- `des work-exhausted-tick --feature-id <id> --project-root . --now <iso8601>` — the third autonomous-loop driving port (the "stops instead of idle-holding" half of the feature). All three ports are auto-ticked once per SessionStart (`handle_session_start()`, fail-open per domain) — no manual invocation needed for the steady-state loop, but direct invocation is the debugging/manual-drain path.

**⚠️ `des verify-worktree-cleanup --repo . --target-branch <b> [--check-only]`** (shipped `parallel-work-cleans-up-after-merge-back`, 2026-07-20) — mechanically removes a worktree it classifies as safe (its `is_ancestor(head, target_branch)` check trivially returns true for a worktree that has made ZERO commits of its own the moment `target_branch` merely advances past its starting point via UNRELATED work — not just a genuine merge). **CONFIRMED DATA-LOSS BUG, fix in flight as of 2026-07-20** (bugfix worktree `bugfix-worktree-cleanup-ancestor-check`) — destroyed one live, in-progress worktree's uncommitted work this session, recovered only via subagent-transcript replay. Until the fix lands and is verified: NEVER run this in ACT mode (i.e. without `--check-only`) against any real, in-progress, or shared worktree — only against a disposable scratch worktree you created for that specific purpose.
