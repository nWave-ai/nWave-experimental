...
11. [ ] Mandate 9 — the representative layer 3+ wiring/sad-path test stays example-only; the smallest sufficient set of separate budgeted semantic-PBT tests exists instead for every declared broad-input/state/failure law that fires (e.g. `BROAD_INPUT_DOMAIN`) — normally one property per distinct law, combined only when one generated observation honestly falsifies all combined laws — each targeting the same promised observation as the representative example at a law-bearing surface whose generated observation carries an explicit preservation map to the promised real-port observation (never a proxy), never replacing or duplicating that example; when no such surface exists, this is `EVIDENCE_GAP` back to DESIGN before authoring, never a silent downgrade to example-only — a declared broad law (e.g. `BROAD_INPUT_DOMAIN`) can never be discharged by examples alone
12. [ ] Mandate 10 — Tier B `test_<feature>_state_machine.py` exists if journey is ≥3 chained scenarios AND input space domain-rich; absent otherwise. Tier-B optionality governs journey-shaped state-machine exploration only — it never gates the Mandate 9 carve-out, which fires independently of Tier B's presence
13. [ ] Mandate 11 — layer 3+ sad paths are named example-based tests (`Bug_<symptom>` or `Sad_<scenario>`); no PBT machinery imported into that sad-path set (a separate declared-law semantic-PBT test per the Mandate 9 carve-out lives in its own file, never merged into these examples)
...
22. [ ] PBT + parametrize density maximized (EXPAND plan v3 §3.A) — every declared broad-input/state/failure law (e.g. `BROAD_INPUT_DOMAIN`) has a `@given` AT carrying an explicit bounded examples/runtime budget, sited per the Mandate 9 carve-out (`nw-test-design-mandates-layered-mechanics`); every finite Cartesian flag combination has a `parametrize` AT; example-based reserved for unique invariants and walking skeleton
