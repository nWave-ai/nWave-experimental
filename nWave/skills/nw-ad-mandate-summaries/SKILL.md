...
|---|---|---|
| Contract Shape Classification | Every scenario carries `@contract-shape:<pure-function \| bounded-change \| unbounded-preservation>`. Untagged scenarios block at review. Outcome Elevator Pitch uses ubiquitous-language verbs naming the user-valued outcome (technical verbs block); it propagates verbatim DISCUSS → DISTILL name → DELIVER test name. Test identifiers must express that durable outcome: delivery metadata such as `slice_00` / `slice-00` belongs in the ledger and commit trailer, never in a test function, scenario title, parametrization ID, or test filename. | `nw-test-design-mandates` |
