...
| `/nw-roadmap` | Create implementation plan (DEPRECATED, classic-only) | Classic-mode only, removed in v4 — under the atdd-pure workflow the feature-delta Slice Plan + per-slice ATs replace roadmap.json. Opt-in under explicit classic authorization. |
