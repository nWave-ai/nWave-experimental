...
3.5. **Reuse-before-create (code-fact)**: before writing a NEW production function/class/module to reach GREEN, resolve code-facts via `nw-code-analysis-port` (Tsunami-first `mcp__tsunami__*`, declared AST->grep fallback, degrade-LOUD — never ad-hoc grep, never a Tsunami hard-dep). Search for an existing symbol that already carries the responsibility about to be implemented.
