...
7. **Stress Analysis** (conditional — `nw-stress-analysis` semantic trigger, role registry `role-skill-loading.yaml`; `--residuality` is that trigger's explicit force-on, not the sole condition) — when triggered: apply complexity-science-based stress analysis (stressors|attractors|residues|incidence matrix|resilience modifications) per the `nw-stress-analysis` skill. When not triggered: skip entirely, do not mention. Gate: activated only on trigger.
