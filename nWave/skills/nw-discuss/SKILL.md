...
- Expectation Charter SEEDS — NOT the charter files (evolution-plan P2.0/P2.1). DISCUSS owes one Slice Plan Value statement per slice that promises observable value; that statement is what `des charter-scaffold` seeds each charter's Intent from at DISTILL-open, before a fresh `nw-product-owner` context FILLS it. Do NOT hand-write files under `docs/product/expectations/` in this wave — the producing tool owns the path, filename and heading dialect (see §Expectation Charter below). A FILLED charter is what ARMS the DELIVER EXAMINE step + the commit-slice examine-verdict gate for that feature (no filled charter → the gate is unarmed and DELIVER falls back to the legacy reviewer audit). <!-- mode-ref-ok -->
- Acceptance criteria are the per-slice `.feature` AT SSOT authored downstream in DISTILL.
...
IF Decision 4 = Yes (default): Execute *jtbd-analysis for {feature-id}, then *journey informed by JTBD artifacts, then *story-map, then *gather-requirements with outcome KPIs. Every user story must include a `job_id` field traceable to `docs/product/jobs.yaml`. **THEN, before closing the wave, make every Slice Plan row that promises observable value carry a Value statement concrete enough to seed a charter Intent — this is a REQUIRED step, not an optional deliverable (§Expectation Charter). Do NOT author charter files here: `des charter-scaffold` produces them at DISTILL-open and a fresh `nw-product-owner` context fills them. Hand-writing them in this wave bypasses the tool's path/naming/heading-dialect guarantees — the exact hand-assembly that caused the 2026-07-12 dialect-mismatch incident. Emit the seeding Value statements in the wave summary so DISTILL can scaffold from them.**
