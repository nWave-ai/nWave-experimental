...
5. **Terminating slice-scoped run** — the canonical command + why it is NEVER a crafter-picked language-specific subset (`pytest tests/...` or similar) is SSOT'd in `nw-crafter-discipline-atdd-pure` § "Terminating Test Run": `des run-slice-ats --repo . --entering-slice <s>`, covering every shipped + entering slice, language-agnostic by construction. The whole-tree contract suite is NOT run per slice; it is run once at the feature-end cycle. The `Gate-Scope:` digest (E2 `--verify-gate-scope`) stays a whole-tree `--collect-only` digest — unchanged.
...
suite via `des run-slice-ats --repo . --entering-slice <s>` (covering every
shipped + entering slice) — NEVER a crafter-picked, language-specific subset
(`pytest tests/{feature-path}/` or similar); see `nw-crafter-discipline-atdd-pure`
§ "Terminating Test Run" for the full rationale, shared verbatim with this cycle.
