...
**Why this is a discipline rule, not a structural detector**: call-graph + body-shape structural matching is a `nw-code-analysis-port` responsibility, not one this rule performs itself — the port does not supply body-shape matching today, so this discipline rule does not depend on it, and any future structural check must go through the port and degrade LOUD (never silent-pass) when the capability is unavailable. Extension Justification is the cheap behavioral nudge that runs at WRITE-time on any LLM-driven crafter regardless of target language. The block makes the bias visible to the reviewer; it does NOT block the write. If the rationale is weak, the reviewer catches it at Phase 4 (Adversarial Review).
