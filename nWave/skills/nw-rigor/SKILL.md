...
- Mutation testing is **not a rigor axis** — it is a project-level policy declared in each project's `CLAUDE.md` § Mutation Testing Strategy (`per-feature | nightly-delta | pre-release | disabled`; unspecified defaults to `disabled`), never a per-feature knob here. `/nw-mutation-test` remains usable as an explicit, on-demand run regardless of strategy.
