...
The WS is an acceptance test on steroids: it proves wiring AND drives implementation of adapters, domain logic, and application services. If the WS AT requires 5 functions to pass, those 5 functions are justified. Subsequent steps that find "already implemented, AT goes GREEN" confirms the WS was well-designed (atdd_pure: AT was active-RED and now passes; classic: equivalent to @skip removal, DEPRECATED per ADR-GV-001 D6). <!-- mode-ref-ok -->
