...
| Process metadata in the test file name (`test_slice_01_*`, `test_slice_02_*`) | The name outlives the slicing decision that produced it — meaningless to a reader without the feature-delta open beside it, and collides across features that both happen to have a "slice-01" | Name the file after what it VERIFIES (`test_shared_git_worktree_refusal.py`), never after which delivery slice authored it. The slice number belongs in the commit trailer (`Slice-Id:`) and the ledger, not the filename. |
