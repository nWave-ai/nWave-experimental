...
**Mode note** <!-- mode-ref-ok -->: the two files above are classic-mode only (ADR-024 era artifacts). Under `workflow.mode = atdd_pure` neither exists — read the AT-completion ledger (`.nwave/telemetry/atdd-pure/{feature-id}.jsonl`) and the feature-delta `## Wave: DISCUSS / [REF] Slice Plan` table instead.
