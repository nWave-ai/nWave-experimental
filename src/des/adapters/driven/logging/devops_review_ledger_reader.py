"""Ledger adapter for the DEVOPS review verdict reader (f-design-devops-review-gate).
...
through ``AtCompletionLedger`` -- one path SSOT). The thin DEVOPS-scoped sibling
of ``DesignReviewLedgerReader`` (DDD-8): same tolerant line scan + keyless
probe, keyed to the ``DevopsReviewVerdict`` event.
...
# reader selects on (sibling of DESIGN_REVIEW_EVENT for the DEVOPS wave).
DEVOPS_REVIEW_EVENT = "DevopsReviewVerdict"
...
class DevopsReviewLedgerReader(DiscussReviewReader):
    """Reads the latest DEVOPS review verdict off the JSONL ledger.
    ...
    def latest(self, project_root: Path, feature_id: str) -> dict[str, object] | None:
        """Tolerant scan: latest ``DevopsReviewVerdict`` for the feature, or None."""
        ...
        for line in ledger.read_text(encoding="utf-8").splitlines():
            ...
            if not isinstance(record, dict):
                continue
            if record.get("event") != DEVOPS_REVIEW_EVENT:
                continue
    ...
    def probe(self, project_root: Path) -> None:
        ...
        Writes a keyless ``DevopsReviewVerdict`` to a probe ledger, reads it back
        ...
        """
        probe_root = project_root / ".nwave" / "devops-gate" / "_probe_review"
        feature_id = "_probe"
    ...
    def _assert_roundtrip_passes(
        ...
        if result.token is not ReviewGateToken.PASS:
            raise RuntimeError(
                "health.startup.refused: devops-review probe round-trip did "
    ...
    def _assert_absent_blocked(
        ...
        if (
            ...
            raise RuntimeError(
                "health.startup.refused: devops-review probe did not block "
