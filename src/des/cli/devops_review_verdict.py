"""DEVOPS review verdict producer (f-design-devops-review-gate slice-02 PRODUCER).

``des record-devops-review``: after the platform-architect-reviewer judges the
DEVOPS artefact, this producer RECORDS the outcome as a ``DevopsReviewVerdict``
record appended to the per-feature AT-completion ledger
``.nwave/telemetry/atdd-pure/{feature_id}.jsonl``. The DEVOPS gate-OUT consumer
(``des verify-devops-review``) reads it back via the ledger reader +
...
producer parameterized to ``wave="devops"`` -- ZERO new verdict logic, the SAME
generic core serves a SECOND wave.

O-4 both-outcomes (reused verbatim from the DESIGN producer): this producer
...
def record_devops_review_verdict(
    repo_root: Path,
    feature_id: str,
    verdict: str,
    ...
    """Append one keyless DevopsReviewVerdict record to the ledger.
    ...
    ``AtCompletionLedger.append_devops_review_verdict``.
    """
    record: dict[str, object] = {
        "event": DEVOPS_REVIEW_EVENT,
    ...
    ledger.append_devops_review_verdict(verdict_fields=verdict_fields)
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="record-devops-review",
        description=(
            "Record a keyless DEVOPS platform-architect review verdict (O-4 "
            "producer). BOTH approved and needs-revision are written -- the "
            "DEVOPS gate-OUT mechanically reads the record to honor a veto."
        ),
    )
