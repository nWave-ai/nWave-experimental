"""Composition root for f-deliver-wave-migration ATs (the SSOT-via-Types mandate).
...
what exercises the registered `skill-normative-gate` subcommand seam — the real
maintainer-facing protocol.
...
application / adapter module and invokes it at the function boundary. The only
production behaviour observed is the subprocess exit code / stdout of the real
gate. The typed Verdict→exit contract lives in `domain_types`, mirrored from
...
`nWave/tasks/nw/deliver.md`, `nWave/skills/nw-tdd-methodology/SKILL.md`) via an
explicit `asset` path. The gate reads those real files; the AT asserts the SHIPPED
exit code — never a fabricated oracle.

This is a PROSE migration (DESIGN: zero new `src/des` module — `feature-delta.md:721`).
These ATs do NOT build or test a conformance engine; the mechanical ConformanceDiff
is DESIGNED-NOT-BUILT and owned by feature 6 (OB-1 → option b/c, `feature-delta.md:573`).
They drive `des skill-normative-gate` to witness that the shipped DELIVER prose
DECLARES the crafter-matches-design discipline.

Why the ATs are ACTIVE-RED today (atdd_pure / ADR-025, NOT @skip):
  • PRESENCE clauses (slice-01/02 + the slice-03 floor): register f-deliver
    crafter-matches-design markers ABSENT from the shipped prose → the real gate
    returns FAIL (exit 1) → the AT expects PASS → AssertionError. DELIVER migrates
    the prose → PASS → green.
  • The LEGACY-ABSENCE clause (slice-03): registers a legacy marker still PRESENT
    ...
    goal) → AssertionError. DELIVER reconciles the legacy prose → FAIL → green.
  • The INDETERMINATE guardrail (AT-6): registers a clause against an unreadable
    asset → the gate returns INDETERMINATE (exit 4) by construction TODAY — the AT
    asserts INDETERMINATE and stays green as a permanent degrade-LOUD guardrail.
Every scenario RUNS and FAILS (or, for the guardrail, asserts INDETERMINATE) with
a semantic AssertionError; none is @skip.
...
crafter prose. The OB-1 crafter-matches-design SEAM (AT-2), the OB-2 K5
named-contradiction bump witness (AT-5), and the C7/C8 language-agnostic AST seam
(AT-7) are DESIGN-declared PROSE seams; each is named here verbatim (a
discriminating marker), driven through its REAL entry point (the dispatcher
subprocess), and witnessed via an observable effect (the exit code / stdout the
gate ships). No seam is left dormant.
"""
...
class DeliverWaveMigrationComposition:
    ...
    def require_shipped_surface_present(self, clause: PresenceClause) -> None:
        ...
        assert asset.is_file(), (
            ...
            f"f-deliver crafter-matches-design prose is migrated INTO this real "
            f"shipped file"
        )
    ...
    def require_legacy_surface_present(self, clause: LegacyClause) -> None:
        ...
        assert asset.is_file(), (
            ...
            f"AT-satisfaction-only prose is reconciled IN this real shipped file"
        )

    def require_floor_surface_present(self, clause: FloorClause) -> None:
        """Pin: the real shipped surface carrying the non-regression floor exists."""
        asset = ASSET_BY_SURFACE[SURFACE_BY_FLOOR[clause]]
        assert asset.is_file(), (
            f"real-surface precondition broken: {asset} is missing — the "
            f"matches-design leg lives in this real shipped file (C9 "
            f"non-regression witness)"
        )

    # --- Manifest authoring (tmp_path-scoped, real JSON, real assets) -----

    def author_presence_manifest(self, clause: PresenceClause) -> None:
        """Author a manifest asserting one f-deliver marker is PRESENT in the surface."""
    ...
    def author_floor_manifest(self, clause: FloorClause) -> None:
        """Author a manifest asserting the matches-design floor stays."""
    ...
    def author_dead_mechanism_manifest(
        ...
        """Author a manifest whose asset cannot be read → INDETERMINATE (AT-6)."""
