"""pytest-bdd configuration for the fix-distill-signoff-feature-end-wiring suite.
...
`CoverageMapVerifiedAtDistillExit` / `CoverageMapVerifiedAtDeliverExit` and
the union read does NOT YET include `coverage_map_touchpoint_events()`.
Every scenario reds for the RIGHT reason -- MISSING_FUNCTIONALITY (production
behaviour absent), not ImportError / FixtureBroken / SetupFailure
(Pre-DELIVER fail-for-right-reason gate).

The collection hook below marks every slice-01 scenario `xfail(strict=False)`
until DELIVER greens it. DELIVER narrows `_RED_SCAFFOLD_SLICES` to
`frozenset()` slice-by-slice at the GREEN phase, one slice at a time, per
the one-at-a-time TDD cadence.
"""
...
def pytest_collection_modifyitems(
    ...
    `strict=False` -- the slice-01 Outline includes rows whose contract
    holds invariantly both pre- and post-wiring (the heartbeat-present
    cases where the missing-record set excludes the touchpoint heartbeat
    ...
    scenario (AT-3, with-both-heartbeats case) seeds both heartbeats so the
    ...
    `fix_oss_environmental_e2e_gate` slice-03 and
    `walking_skeleton_feature_end_wiring` slice-01 patterns. The RED
    cadence intent is preserved by the marker; the genuine RED rows
    (heartbeat-missing Outline rows + CLI parity scenario) fail organically
    today and will be lifted by DELIVER's frozenset+union extension.
    """
