"""Composition root for f-devops-wave-migration ATs (the SSOT-via-Types mandate).
...
(`nWave/agents/nw-platform-architect.md`, `nWave/tasks/nw/devops.md`,
`nWave/skills/nw-infrastructure-and-observability/SKILL.md`) via an explicit
`asset` path. The gate reads those real files; the AT asserts the SHIPPED exit
code — never a fabricated oracle.

Why the ATs are ACTIVE-RED today (atdd_pure / ADR-025, NOT @skip):
  • PRESENCE clauses (slice-01/02 + slice-03 floor): register f-devops markers
    ABSENT from the shipped prose → the real gate returns FAIL (exit 1) → the AT
    expects PASS → AssertionError. DELIVER migrates the prose → PASS → green.
...
nw-platform-architect / nw-devops / nw-infrastructure-and-observability normative
prose. It is named here verbatim, driven through its REAL entry point (the
dispatcher subprocess), and every AT asserts an observable effect (the exit code
/ stdout the gate ships). The OB-1 security-gate seam + OB-2 Tier-B advisory are
DESIGN-declared prose seams; AT-6 and AT-7 each name THAT exact seam (a
discriminating marker) and witness it through the gate. No seam is left dormant.
"""
...
class DevopsWaveMigrationComposition:
    ...
    def require_shipped_surface_present(self, clause: PresenceClause) -> None:
        ...
        assert asset.is_file(), (
            ...
            f"f-devops normative prose is migrated INTO this real shipped file"
        )
    ...
    def require_legacy_surface_present(self, clause: LegacyClause) -> None:
        ...
        assert asset.is_file(), (
            ...
            f"free-form decision-question prose is removed FROM this real shipped file"
        )
    ...
    def require_floor_surface_present(self, clause: FloorClause) -> None:
        ...
        assert asset.is_file(), (
            f"real-surface precondition broken: {asset} is missing — the "
            f"KPI-traced observability prose lives in this real shipped file "
            f"(C7 non-regression witness)"
        )

    # --- Manifest authoring (tmp_path-scoped, real JSON, real assets) -----

    def author_presence_manifest(self, clause: PresenceClause) -> None:
        """Author a manifest asserting one f-devops marker is PRESENT in the surface."""
    ...
    def author_floor_manifest(self, clause: FloorClause) -> None:
        """Author a manifest asserting the KPI-traced observability floor stays."""
    ...
    def author_dead_mechanism_manifest(
        ...
        """Author a manifest whose asset cannot be read → INDETERMINATE (AT-6/AT-8)."""
