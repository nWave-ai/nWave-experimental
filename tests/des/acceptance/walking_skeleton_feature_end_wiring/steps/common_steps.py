"""Shared step vocabulary for the fix-walking-skeleton-feature-end-wiring suite.
...
from .composition import WalkingSkeletonFeatureEndWiringComposition
...
@pytest.fixture
def composition() -> WalkingSkeletonFeatureEndWiringComposition:
    """The production composition root, fresh per scenario."""
    return WalkingSkeletonFeatureEndWiringComposition()
...
def given_ledger_state(
    composition: WalkingSkeletonFeatureEndWiringComposition, ledger_state: str
) -> None:
...
def when_u4_enforcer_runs(
    composition: WalkingSkeletonFeatureEndWiringComposition,
) -> None:
    composition.when_u4_enforcer_runs()


@when("the verify_deliver_integrity CLI runs on the feature")
def when_cli_runs(composition: WalkingSkeletonFeatureEndWiringComposition) -> None:
...
@then(
    parsers.parse(
        "the walking-skeleton heartbeat is {missing_outcome} in the missing-record set"
    )
)
def then_missing_outcome(
    composition: WalkingSkeletonFeatureEndWiringComposition, missing_outcome: str
) -> None:
...
@then("the verdict names the walking-skeleton heartbeat as a missing required record")
def then_cli_names_walking_skeleton(
    composition: WalkingSkeletonFeatureEndWiringComposition,
) -> None:
    composition.then_cli_names_walking_skeleton_heartbeat_missing()
