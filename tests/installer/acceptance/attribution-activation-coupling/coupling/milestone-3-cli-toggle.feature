...
Feature: Toggling attribution drives the hook and the preference, not the settings credit
  ...
  # AB-7 — on: preference on, NO settings credit written.
  @ab-7 @driving_port @contract-shape:bounded-change
  Scenario: Turning attribution on records the preference
    Given a active repo
    When the operator turns attribution on
    Then the attribution preference is recorded as on
    And no managed attribution credit is written to the Claude settings

  # AB-6 — off: execution guard intact; preference off; legacy cleaned.
  @ab-6 @driving_port @contract-shape:bounded-change
  Scenario: Turning attribution off leaves the execution guard intact
    Given a active repo
  ...
  # AB-6 — uninstall preserves a user value.
  @ab-6 @error @contract-shape:bounded-change
  Scenario: Uninstall preserves a user credit
    Given a active repo
