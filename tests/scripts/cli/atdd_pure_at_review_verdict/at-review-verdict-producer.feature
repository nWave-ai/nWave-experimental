...
Feature: DISTILL records a keyless AT-review verdict for a reviewed slice
  ...
  as a verdict in the feature's AT-completion ledger -- the single audit record
  that lets the DELIVER entry gate later confirm the slice was reviewed before
  any crafter is dispatched.

  Post-demotion the record carries the reviewer identity, the AT-set binding,
  and the content seal (SHA-256 over sorted scenario bodies) but NO hmac_sha256
  field. Key absence is a non-event: the producer never resolves a signing key.
  The content seal (at_content_hash) is the load-bearing tamper-evidence; the
  signed-field HMAC is an SF-tier concern removed from the OSS producer.
  ...
  # Contract: the producer writes the veto fields (reviewer_agent_id, slice_id,
  # verdict, at_ids, at_content_hash, timestamp) and NO hmac_sha256 field.
  ...
  Scenario: An approved slice gets a keyless verdict appended to the ledger
  ...
  Scenario Outline: The verdict covers a reviewed AT set of any size
    ...
    When the designer records the AT-review verdict for the entering slice
    Then the verdict lists <reviewed test count> reviewed test identifiers
    And the content fingerprint matches the reviewed acceptance tests
