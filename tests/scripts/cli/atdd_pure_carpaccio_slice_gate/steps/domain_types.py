...
class ATReviewRejectReason(str, Enum):
    """The closed four-value ``ATReviewGateRejected`` reason set (post-demotion).
...
class ATReviewRecordState(str, Enum):
    ...
    Each state isolates exactly one assertion-5 failure condition, plus the
    APPROVED happy state. No signing key is provisioned for any state.

    APPROVED_VALID  -- a keyless APPROVED record matching the slice's current
                       AT set by id and content hash; assertion 5 PASS.
    NO_RECORD       -- ledger has no record for the entering slice -> reason absent.
    NEEDS_REVISION  -- a record whose verdict is NEEDS_REVISION
                       -> reason not-approved.
...
# iterates this mapping so all four assertion-5 rejection branches are covered.
