"""Composition root for the feature-dependency-justification acceptance slice.
...
epic-delta / feature-delta fixture. Business logic lives here as the single
source of truth; step bodies delegate to
``FeatureDependencyJustificationComposition`` methods and never inline logic.
...
``des validate-feature-delta --require-feature-plan --format=json`` (six of
the seven scenarios) and ``--require-slice-plan --format=json`` (the
isolation scenario) -- via ``main(argv)`` (Layer 3 subprocess/FS
acceptance), exactly the pattern the shipped
``atdd_pure_validate_feature_delta_slice_dependency_justification`` sibling
suite already established one granularity down (EXTEND, not reinvent -- see
this feature's own Reuse Analysis).
...
``--require-feature-plan --format=json`` the CLI's closed set gains ONE
token this feature introduces:
...
Under ``--require-slice-plan --format=json`` the closed set is UNCHANGED
(D-6/CT-4 -- this feature's isolation scenario proves
``unjustified-slice-dependency`` keeps firing on its own surface, undisturbed
by the feature-grain flip):
...
brand-new-flag slice, ``--require-feature-plan --format=json`` already ships
(discuss-epic-mode slice-01) -- so ONLY the scenarios exercising genuinely
NEW behaviour are RED on the current tip:

- the "declared feature dependency, empty Justification -> rejected"
  scenario is RED today: ``_FEATURE_PLAN_SPEC.enforce_dependency_justification``
  is ``False`` on the current tip, so the per-row dependency check never runs
  in feature-plan mode -- a ``depends-on`` row with an empty Justification
  cell currently comes back ``accepted``, the wrong verdict.
- the "structurally malformed feature-dependency row -> malformed Feature
  Plan" scenario is RED today for the same root cause: with the dependency
  check disabled, a row missing its Justification column entirely is
  silently accepted rather than rejected.

Every other scenario pins EXISTING, already-shipped behaviour (a
non-dependency row is already accepted today, a justified-dependency row is
already accepted today since feature-plan mode does not yet look at
Annotation/Justification content at all, and the slice-plan isolation is
already true today since row 1 shipped it) -- they are non-regression
guards, correctly green now and required to stay green once slice-01 lands.
This is the expected shape for an INCREMENTAL verdict-set extension (mirrors
row 1's own composition module) -- not every scenario in an atdd_pure slice
need be individually RED when the slice adds one classification arm to an
already-accepted surface.
"""
...
# --require-feature-plan and --require-slice-plan already exist
# (discuss-epic-mode slice-01 / slice-06); slice-01 of THIS feature EXTENDS
# the feature-plan mode's per-row classification with the new dependency
# check, reusing the shared classifier the slice-plan mode already exercises.
from des.cli.validate_feature_delta import (
    main as validate_feature_delta_main,
)

from .domain_types import CheckMode, DependencyVerdict, EpicId, SecondRowShape
...
# The fixed first row every Feature Plan fixture carries -- keeps the
# pre-existing cohesion-MECC floor (`_classify_slice_cohesion`) from vetoing
# a plan whose only row happens to be `@infrastructure`, so each scenario
# isolates exactly the second row's Annotation/Justification shape. Named
# after Domain Example 3's anchor feature (Sofia Marchetti, billing-
# reliability epic).
_ANCHOR_FEATURE_ROW = (
    "| webhook-retry-core | Maintainer ships the anchor feature | pending | "
    "@walking_skeleton | thinnest end-to-end vertical |"
)

# Second-row builders, one per `SecondRowShape`. Module-level dispatch keeps
# `provision_feature_plan` a single typed lookup (Mandate-12 criterion 3).
_SECOND_FEATURE_ROW_BY_SHAPE: dict[SecondRowShape, str] = {
    SecondRowShape.NO_ANNOTATION: (
        "| dunning-emails | Maintainer ships the dependent feature | pending |  |  |"
    ),
    ...
    SecondRowShape.DEPENDENCY_JUSTIFIED: (
        "| dunning-emails | Maintainer ships the dependent feature | pending | "
        "depends-on webhook-retry-core | "
        "consumes the retry policy webhook-retry-core defines |"
    ),
...
# The Slice Plan fixture (slice granularity, feature-delta) used ONLY by the
# isolation scenario -- proves the feature-grain flip does not disturb the
# already-shipped slice-grain rule (D-6/CT-4, the mirror-image of row-1's own
# isolation scenario).
_SLICE_PLAN_ANCHOR_ROW = (
    "| slice-01 | Operator ships the walking skeleton | pending | "
    "@walking_skeleton | thinnest end-to-end vertical |"
)
_SLICE_PLAN_UNJUSTIFIED_DEPENDENCY_ROW = (
...
class ValidationResult:
    ...
    The ``--require-feature-plan --format=json`` / ``--require-slice-plan
...
@dataclass
class FeatureDependencyJustificationComposition:
    ...
    def _feature_delta_path(self) -> Path:
        return (
            ...
            / "docs"
            / "feature"
            / "parallel-by-default-feature-plan"
            / "feature-delta.md"
        )
    ...
    def provision_feature_plan(self, shape: SecondRowShape) -> None:
        """Write an epic-delta whose Feature Plan's second row is `shape`."""
        second_row = _SECOND_FEATURE_ROW_BY_SHAPE[shape]
        body = (
            "# Epic Delta: feature-dependency-justification fixture\n\n"
            "## Wave: DISCUSS / [REF] Feature Plan\n\n"
            "| Feature | Value statement | Status | Annotation | Justification |\n"
            "|---------|-----------------|--------|------------|---------------|\n"
            f"{_ANCHOR_FEATURE_ROW}\n{second_row}"
        )
        self._epic_delta_path.parent.mkdir(parents=True, exist_ok=True)
        self._epic_delta_path.write_text(body, encoding="utf-8")
        self.active_path = self._epic_delta_path

    def provision_slice_plan_with_unjustified_dependency(self) -> None:
        """Write a feature-delta whose Slice Plan carries an unjustified
        `depends-on` row -- the isolation fixture (D-6/CT-4)."""
        body = (
            "# Feature Delta: feature-dependency-justification isolation fixture\n\n"
            "## Wave: DISCUSS / [REF] Slice Plan\n\n"
            "| Slice | Value statement | Status | Annotation | Justification |\n"
            "|-------|-----------------|--------|------------|---------------|\n"
            f"{_SLICE_PLAN_ANCHOR_ROW}\n{_SLICE_PLAN_UNJUSTIFIED_DEPENDENCY_ROW}"
        )
    ...
    def run_check(self, mode: CheckMode) -> ValidationResult:
        ...
        target = (
            self._epic_delta_path
            if mode is CheckMode.FEATURE_PLAN
            else self._feature_delta_path
        )
