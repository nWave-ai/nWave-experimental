"""Domain types for the slice-dependency-justification acceptance slice.
...
# A kebab-case feature identifier (e.g. "parallel-by-default-slice-plan").
...
class CheckMode(str, Enum):
    ...
    SLICE_PLAN   -- ``--require-slice-plan --format=json`` against a
                    feature-delta's `[REF] Slice Plan` (this feature's own
                    surface).
    FEATURE_PLAN -- ``--require-feature-plan --format=json`` against an
                    epic-delta's `[REF] Feature Plan` -- driven ONLY by the
                    isolation scenario, which proves the new rule does not
                    leak one granularity up (D-6 / Out-of-Scope row 3).
    """
...
class DependencyVerdict(str, Enum):
    ...
    collide between modes, so one enum safely maps either. The slice-plan
...
class SecondRowShape(str, Enum):
    ...
    COUPLED                  -- `@coupled`, empty Justification.
    ...
    DEPENDENCY_UNJUSTIFIED   -- `depends-on slice-01`, empty Justification
                                (Domain Example 3 -- the new rejection).
    DEPENDENCY_MALFORMED_ROW -- `depends-on slice-01` Annotation, but the row
                                itself drops the Justification column
                                entirely (fewer than five cells) -- the
                                "malformed token fails LOUD" guard.
    """
...
SECOND_ROW_SHAPE_BY_ANNOTATION_PHRASE: dict[str, SecondRowShape] = {
    ...
    "@coupled": SecondRowShape.COUPLED,
}
